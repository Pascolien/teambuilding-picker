...
export default function AddActivityButton({
  ...
  function submit(){
    ...
    onAdd({ title: title.trim(), url: url.trim(), description: description.trim() || null, tags: tagsFrom(tags) })
    setTitle(''); setUrl(''); setDescription(''); setTags(''); setOpen(false)
  }

  return (<>
    <button onClick={()=>setOpen(true)} className="px-3 py-2 rounded bg-slate-900 text-white hover:bg-slate-800 flex items-center gap-2"><Plus className="w-4 h-4"/>Ajouter</button>
    {open && (
...
function tagsFrom(raw:string){ const arr = raw.split(',').map(s=>s.trim()).filter(Boolean); return arr.length? arr : null }
